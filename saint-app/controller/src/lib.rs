...
use domain::boundaries::{
    CreateSaintError, SaintDbGateway, SaintMutationInputBoundary, SaintMutationRequest,
...
pub async fn create_saint(saint: &Saint) -> Result<openapi::saint::Saint, CreateSaintError> {
    let client = db_postgres::connect().await;
